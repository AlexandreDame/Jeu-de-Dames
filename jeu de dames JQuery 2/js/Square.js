	...
	constructor(x, y, $elt, droppable, platform, game) {
		...
		this.$elt.droppable({
  			...
  			drop: ( event, ui ) => {
    			...
    			if 
                (   
                    !this.droppable || this.pawn !== 'none' || this.x === platform.pawnPlayed.x 									||
                    ...
                    (   platform.pawnPlayed.color === 'white' && 
    					this.x === platform.pawnPlayed.x + 2  && 
    					platform.rows[this.y-1][this.x-1].pawn.color != 'black'
    				)
                    																												||
    				(   platform.pawnPlayed.color === 'white' && 
    					this.x === platform.pawnPlayed.x - 2  && 
    					platform.rows[this.y-1][this.x+1].pawn.color != 'black'
                     																												...
                     																												||
    				(   platform.pawnPlayed.color === 'black' && 
    					this.x === platform.pawnPlayed.x + 2  && 
    					platform.rows[this.y+1][this.x-1].pawn.color != 'white'
    				)
                																												||
    				(   platform.pawnPlayed.color === 'black' && 
    					this.x === platform.pawnPlayed.x - 2  && 
                    ...
                    $('.dialog').empty().append('Mouvement non autorisé').slideDown();
